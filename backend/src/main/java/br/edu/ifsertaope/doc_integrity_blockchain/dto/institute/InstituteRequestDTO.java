...
public record InstituteRequestDTO(
        @JsonProperty("institute_name")
        @NotBlank(message = "institute_name is required")
        @Size(max = 255, message = "institute_name must have at most 255 characters")
        String institute_name,

        @JsonProperty("institute_identifier")
        @NotBlank(message = "institute_identifier is required")
        @Pattern(regexp = "(^\\d{11}$|^\\d{14}$)", message = "The fields must have 11 (CPF) or 14 (CNPJ) numeric characters")
        String institute_identifier,
